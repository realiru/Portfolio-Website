...
function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event) => {
      ...
      if (!svg) return; // Check if the SVG element exists
      ...
      const updateEyePosition = (eye, offsetX, offsetY) => {
        ...
        const eyeX = offsetX + Math.cos(angle) * 4; // Adjust the multiplier to control the movement range
        const eyeY = offsetY + Math.sin(angle) * 4; // Adjust the multiplier to control the movement range
